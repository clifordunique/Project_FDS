...
[AddComponentMenu("Pathfinding/AI/AIPathCustom (2D,3D)")]
[HelpURL("http://arongranberg.com/astar/docs/class_a_i_path.php")]
public class AIPathCustom : AIBase {
	...
	protected override void MovementUpdate (float deltaTime) {
		...
		if (!interpolator.valid) {
            ...
            //TODO: maybe it's the good idea, continue on this
            Vector3 currentVelocity = Vector3.one;
            Vector3 rawDirection = Vector3.zero;

            rawDirection = dir;
            transform.position += rawDirection * speed * Time.deltaTime;
            //tr.position = Vector3.SmoothDamp(tr.position, targetPoint, ref currentVelocity, .5f, speed);
